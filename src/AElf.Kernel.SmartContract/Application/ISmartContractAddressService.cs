...
namespace AElf.Kernel.SmartContract.Application
{
    public interface ISmartContractAddressService
    {
        ...
        Task<SmartContractAddress> GetSmartContractAddressAsync(IChainContext chainContext, Hash name);
        Task<bool> CheckSmartContractAddressIrreversibleAsync(SmartContractAddress smartContractAddress);
    ...
    public class SmartContractAddressService : ISmartContractAddressService, ISingletonDependency
    {
        ...
        public async Task<SmartContractAddress> GetSmartContractAddressAsync(IChainContext chainContext, Hash name)
        {
            var smartContractAddress = await _smartContractAddressProvider.GetSmartContractAddressAsync(chainContext, name);
            if (smartContractAddress != null) return smartContractAddress;
            var address = await GetSmartContractAddressFromStateAsync(chainContext, name);
            return new SmartContractAddress
            {
                Address = address
            };
        }
        
        public async Task<bool> CheckSmartContractAddressIrreversibleAsync(SmartContractAddress smartContractAddress)
        {
            if (smartContractAddress == null || smartContractAddress.BlockHeight == 0) return false;
