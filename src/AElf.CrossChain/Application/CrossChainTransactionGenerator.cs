...
namespace AElf.CrossChain.Application
{
    internal class CrossChainTransactionGenerator : ISystemTransactionGenerator
    {
            ...
            Hash previousBlockHash)
        {
            ...
            generatedTransactions.Add(GenerateNotSignedTransaction(from, crossChainTransactionInput.MethodName, new BlockIndex
            {
        ...
        /// <returns></returns>
        private Transaction GenerateNotSignedTransaction(Address from, string methodName, IBlockIndex blockIndex, ByteString bytes)
        {
            var address = AsyncHelper.RunSync(() => _smartContractAddressService.GetAddressByContractNameAsync(
                new ChainContext
                {
